...
const ChatSelector = ({ rooms }) => {
    ...
    return (
        <div className='flex flex-col justify-center'>
            <h3 className='flex justify-center'>Have an Interest? Pick a Chatroom or creat your own!</h3>
            ...
            <div className='flex justify-center'>
                <div className='grid grid-cols-2 gap-5 p-10 place-content-center '>
                    ...
                    {rooms?.map((room) => (
                        <div key={room.id} >
                            <h3  id={room.id} >
                                <Link className='p-3 bg-sky-700 rounded-md flex justify-center items-center hover:scale-[1.02] font-bold h-[200px] w-[200px] transition ease-in-out duration-300' to={`/rooms/${room.colURL}`}>{room.colName} {room?.colEmoji}</Link>
                            </h3>
                        </div>
            ...
            <AnimatePresence
                ...
                onExitComplete={() => null}
            >
                {modalIsOpen && <Modal modalIsOpen={modalIsOpen} handleClose={closeModal} />}
            </AnimatePresence>
